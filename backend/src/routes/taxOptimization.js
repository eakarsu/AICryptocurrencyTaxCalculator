/**
 * Custom Feature: tax-optimization
 * ML-driven tax minimization strategies
 *
 * POST /api/ai/tax-optimization
 * Auth required. Generated as part of Custom Feature Suggestions scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_TAX_OPTIMIZATION_KEY
...
const FEATURE_KEY = process.env.FEATURE_TAX_OPTIMIZATION_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: ML-driven tax minimization strategies.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AICryptocurrencyTaxCalculator - tax-optimization',
    },
    body: JSON.stringify({
...
router.post('/tax-optimization', auth, async (req, res) => {
  try {
    ...
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_TAX_OPTIMIZATION_KEY');
    const ai = await callLLM({ feature: 'tax-optimization', goal: 'ML-driven tax minimization strategies', input: payload });
    // No persistence wired.
    return res.json({
      ok: true,
      feature: 'tax-optimization',
      endpoint: '/api/ai/tax-optimization',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
...
router.get('/tax-optimization/health', (req, res) => {
  res.json({
    feature: 'tax-optimization',
    endpoint: '/api/ai/tax-optimization',
