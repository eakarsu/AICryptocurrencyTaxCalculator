 ...
 * Slug: regulatory-scenario-modeling
 * Backend: /api/cf-regulatory-scenario-modeling
 */
export default function RegulatoryScenarioModelingPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/cf-regulatory-scenario-modeling', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>Regulatory scenario modeling</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>Model impact of changing crypto tax regulations (long-term vs. short-term capital gains, wash-sale rules) on tax bill.</p>
